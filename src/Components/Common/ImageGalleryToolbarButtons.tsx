...
export function GalleryCloseButton(props: { onClose: () => void }) {
  return (
    <button
...
export function GalleryLocationButton(props: {
  location: string;
  onShowLocation: (location: string) => void;
}) {
  return (
    <button
      onClick={() => props.onShowLocation(props.location)}
...
export function GalleryDeleteButton(props: {
  onDelete: () => void;
  ...
  return (
    <button
      ...
      className={`${btn} bg-red-500 hover:bg-red-600 ${
        props.showLocation ? "right-32" : "right-20"
      }`}
      title="Delete image"
